...
import ProfileSkill from './modules/skill/Main';
...
function Main() {
  ...
  const getSeekerDetail = async () => {
    const res = await getSeekerById(id);
    setSeeker(res);
  };
  ...
  return (
    <>
      <Link to="/seekers" className="flex items-center mt-8 intro-y">
        <Lucide icon="ChevronsLeft" className="w-4 h-4 mr-2" />
        <h2 className="mr-auto font-medium text-md">Shift Seeker Profile</h2>
      </Link>
      {!!seeker && (
        <TabGroup>
          <div className="px-5 pt-5 mt-5 intro-y box">
            <div className="flex flex-col pb-5 -mx-5 border-b lg:flex-row border-slate-200/60 dark:border-darkmode-400">
              <div className="flex items-center justify-center flex-1 px-5 lg:justify-start">
                ...
                <div className="ml-5">
                  ...
                  <div className="text-slate-500">Shift Seeker</div>
                </div>
              </div>
            ...
            <TabList className="flex-col justify-center text-center nav-link-tabs sm:flex-row lg:justify-start">
              ...
              <Tab fullWidth={false} className="flex items-center py-4 cursor-pointer">
                <Lucide icon="Wrench" className="w-4 h-4 mr-2" /> Skill
              </Tab>
            </TabList>
          </div>
          <TabPanels className="mt-5">
            ...
            <TabPanel>
              <div className="grid grid-cols-12 gap-6">
                <ProfileGroup seeker={seeker} />
              </div>
            </TabPanel>
            <TabPanel>
              <div className="grid grid-cols-12 gap-6">
                <ProfileSkill seeker={seeker} />
              </div>
            </TabPanel>
          </TabPanels>
        </TabGroup>
